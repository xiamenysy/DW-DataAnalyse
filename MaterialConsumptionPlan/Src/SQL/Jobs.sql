...
DROP TABLE INV_SAP_ITEM_SO_STAT_TMP;
SELECT * FROM INV_SAP_ITEM_SO_STAT_TMP;
CREATE TABLE INV_SAP_ITEM_SO_STAT_TMP AS
SELECT SALES_OPEN.ID,
  SALES_OPEN.SALESDOC,
  SALES_OPEN.MATERIAL,
  SALES_OPEN.PLANT,
  SALES_OPEN.OPEN_QTY,
  SALES_OPEN.MAX_COMMIT_DATE,
  ITEM_LT.LEAD_TIME
FROM
  (SELECT MATERIAL
    ||'_'
...
SELECT ID,
  ...
  (LT_WEEKS13_OPEN_QTY - LT_OPEN) AS LT_WEEKS13_OPEN_QTY,
  WEEKS13_OPEN AS WEEKS13_OPEN_QTY
  FROM 
(
SELECT ID,
  MATERIAL,
  ...
  SUM(LT_WEEKS13_OPEN_QTY) AS LT_WEEKS13_OPEN_QTY,
  SUM(WEEKS13_OPEN)    AS WEEKS13_OPEN
FROM
  (SELECT ID,
    SALESDOC,
    MATERIAL,
    ...
    LEAD_TIME,
    CASE
      WHEN MAX_COMMIT_DATE < SYSDATE - 1
      THEN OPEN_QTY
      ELSE 0
    END PASS_DUE,
    CASE
      WHEN MAX_COMMIT_DATE < SYSDATE + LEAD_TIME
      THEN OPEN_QTY
      ELSE 0
    END LT_OPEN,
    CASE
      WHEN MAX_COMMIT_DATE < SYSDATE + 91
      THEN OPEN_QTY
      ELSE 0
    END LT_WEEKS13_OPEN_QTY,
    CASE
      WHEN MAX_COMMIT_DATE > SYSDATE + 91
      THEN OPEN_QTY
      ELSE 0
    END WEEKS13_OPEN
  FROM INV_SAP_ITEM_SO_STAT_TMP
  WHERE MATERIAL = '1756-IB16 A'
  AND PLANT      = '5040'
  )
GROUP BY ID,
  MATERIAL,
...
SELECT * FROM INV_SAP_ITEM_SO_STAT_TMP WHERE MATERIAL = '1756-IB16 A' AND PLANT = '5040'; 
SELECT DISTINCT MAX_COMMIT_DATE FROM INV_SAP_SALES_VBAK_VBAP_VBUP;
