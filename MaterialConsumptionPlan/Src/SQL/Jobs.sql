...
SELECT MATERIALID
  ...
  MATERIALID                 AS MATERIAL,
  PLANTID                    AS PLANT,
  BSART_PURCHDOCTYPE         AS PURCH_DOC_TYPE,
  STATU_STATUSPURCHDOC       AS STATUS,
  LIFNR_VENDORNO             AS VENDOR,
  EBELNPURCHDOCNO            AS PURCHASE_ORDER,
  EBELPPURCHITEMNO           AS LINE,
  ETENRPURCHDELIVSCHLINE     AS PURCH_DELIVERY_SCH_LINE,
  MENGESCHEDULEDQTY          AS SCHEDULE_QTY,
  AMENGPREVIOUSQTY           AS PREVIOUS_QTY,
  WEMNGRECEIVEDQTY           AS RECEIVED_QTY,
  WAMNGISSUEDQTY             AS ISSUED_QTY,
  BEDATSCHEDULELINEORDERDATE AS SCHEDULE_LINE_ORDER_DATE,
  EINDTPURCHITEMDELIVDATE    AS PURCH_ITEM_DELIVERY_DATE,
  SLFDTSTATDELIVERYDATE      AS START_DELIVERY_DATE,
  BUDATPOSTINGDATE           AS POST_ING_DATE,
  TOTALRECIEVEDYQTY          AS TOTAL_RECEIVED_QTY,
  HEADER_DELETED_FLAG        AS HEADER_DELETED_FLAG,
  ELIKZDELIVERYCOMPLETEDIND  AS DELIVERY_COMPLETE,
  SCHEDULELINEWEEK           AS SCHEDULE_LINE_WEEK,
  DELIVERYCOMPLETE           AS DELIVERY_COMPLETE,
  COMMITTED_DATE             AS COMMITTED_DATE,
  COMMITTEDQTY               AS COMMITTED_QTY,
  REDUCED_QTY                AS REDUCE_QTY,
  POTOTALREDUCEDQTY          AS PO_TOTAL_REDUCED_QTY,
  GLMNG_DELIVERED_STOQTY     AS DELIVERY_STO_QTY,
  COMMIT_CONFIRM_SUM         AS COMFIRM_SUM,
  PO_SO_SA_FLAG              AS FLAG,
  CREATED_DATE               AS CREATE_DATE,
  POITEMCUM                  AS PO_ITEM_CUM,
  POITEMCOMMITTCUM           AS PO_ITEM_COMMITT_CUM,
  WAERS_CURRENCYKEY          AS CURRENCY,
  NETWRNETORDERVALUE         AS ORE,
  AVG26_USAGE_QTY            AS AVG26_USAGE_QTY,
  LT                         AS LEAD_TIME,
  MRP_CONTROLLER_DISPO       AS MRP_CONTROLLER_DISPO,
  MRP_CONTROLLER             AS MRP_CONTROLLER,
  SPC_PROC_KEY               AS SPC_PROC_KEY,
  PURCH_GROUP                AS PURCHAE_GROUP,
  OH_QTY                     AS OH_QTY,
  STRATEGY_GRP               AS STRATEGY_GRP,
  UNIT_COST                  AS UNIT_COST,
  VENDORNAME                 AS VENDOR_NAME,
  PERFORMANCEONTIME          AS ON_TIME,
  MENGESCHEDULEDQTY_BASE_UOM AS SCHEDULEDQTY_BASE_UOM,
  WEMNGRECEIVEDQTY_BASE_UOM  AS RECEIVEDQTY_BASE_UOM,
  GRT                        AS GRT,
  PURCH_REQ                  AS PR,
  PURCH_REQ_ITEM             AS PR_ITEM,
  LAST_SHIP_DATE             AS LAST_SHIP_DATE,
