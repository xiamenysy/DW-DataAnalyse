...
FROM
  ...
  FROM INV_SAP_PP_PO_HISTORY
  WHERE DELIVERYCOMPLETE        IS NULL AND PLANTID ='5040'
...
LEFT JOIN
  ...
  LEFT JOIN
    (SELECT ID,
      ...
      PROD_BU,
      MATL_TYPE
    FROM VIEW_INV_SAP_PP_OPT_X where id = '100-C09D400 A_5040' 
    )ITEM_BASIC
  ON ITEM_BASIC.ID                  = STOCK_IN_TRAINST.ID
  )TRANSIT_ITEM_BASIC 
  ON OPEN_PO.ID = TRANSIT_ITEM_BASIC.ID;

select * from VIEW_INV_SAP_ITEM_X where id = '100-C09D400 A_5040' MATERIAL = '100-C09D400 A' and plant = '5040';
--DELIVERY RECORD
