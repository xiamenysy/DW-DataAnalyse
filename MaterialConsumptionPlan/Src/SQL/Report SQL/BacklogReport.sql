...
SELECT SALES_PP.ID AS ID,
  SALES_PP.SALESDOC,
  SALES_PP.SALESDOCITEM,
  SALES_PP.MATERIAL,
  SALES_PP.PLANT,
  SALES_PP.LINECREATEDATE,
  SALES_PP.PRODHIER,
  SALES_PP.ORDERQTY,
  SALES_PP.CONFIRMQTY,
  SALES_PP.DELIVPRIO,
  SALES_PP.LINECREATEDCOUNTER,
  SALES_PP.PROFITCENTER,
  SALES_PP.SALESPRICE,
  SALES_PP.COST,
  SALES_PP.STOCKSTATUS,
  SALES_PP.MAX_COMMIT_DATE,
  SALES_PP.REQTYPE,
  SALES_PP.PSPT,
  SALES_PP.HEADERCREATECOUNTER,
  SALES_PP.MAX_CONFIRM_DATE,
  SALES_PP.SOLD_TO,
  SALES_PP.SALESOFF,
  SALES_PP.TELE,
  SALES_PP.DELIVSTATUS,
  SALES_PP.OVERALLDELIVSTATSU,
  SALES_PP.REJECTSTATUS,
  SALES_PP.DW_DATE,
  SALES_PP.SALESDOCTYPE,
  SALES_PP.REJECTREASON_ABGRU,
  SALES_PP.SALEDOCITEMCATEGORY_PSTYV,
  SALES_PP.REQ_DELIVERYQTY_LSMENG,
  SALES_PP.SHIPFROM_VSTEL,
  SALES_PP.SALES_ORG,
  SALES_PP.CURRENCY,
  SALES_PP.NUMERATOR,
  SALES_PP.DENOMINATOR,
  SALES_PP.OPEN_QTY,
  SALES_PP.OPEN_QTY_BASE_UOM,
  SALES_PP.LST_ACT_GI_DATE,
  SALES_PP.LST_DELV_CREATED_DATE,
  SALES_PP.EXCHANGE_RATE_TO_USD,
...
SELECT EBELNPURCHDOCNO,MATERIALID, PLANTID,PO_SO_SA_FLAG, SUM(COMMITTEDQTY) AS OPEN_QTY
FROM DWQ$LIBRARIAN.INV_SAP_PP_PO_HISTORY@ROCKWELL_DBLINK
WHERE PLANTID  = '5050' AND PO_SO_SA_FLAG = 'STO' AND DELIVERYCOMPLETE IS NULL group by MATERIALID, PLANTID,EBELNPURCHDOCNO,PO_SO_SA_FLAG; 

---Line count 

SELECT 
EBELNPURCHDOCNO||'_'||MATERIALID AS ID,
MATERIALID,
  PLANTID,
  SUM(PLANTID)/PLANTID AS LINECOUNT
FROM DWQ$LIBRARIAN.INV_SAP_PP_PO_HISTORY@ROCKWELL_DBLINK WHERE PLANTID = '5050'
GROUP BY EBELNPURCHDOCNO||'_'||MATERIALID,MATERIALID,PLANTID; --,EBELNPURCHDOCNO--,ETENRPURCHDELIVSCHLINE;
...
SELECT * FROM INV_SAP_PO_DETAIL;

CREATE TABLE INV_SAP_PO_DETAIL AS 
SELECT * FROM
(
SELECT 
EBELNPURCHDOCNO||'_'||MATERIALID AS ID,
...
FROM DWQ$LIBRARIAN.INV_SAP_PP_PO_HISTORY@ROCKWELL_DBLINK 
WHERE PLANTID  IN ('5050','5040', '5070', '5100', '5110', '5140', '5200') AND DELIVERYCOMPLETE IS NULL
)PO_HIS
LEFT JOIN
(
SELECT 
...
MATERIALID
  PLANTID,
  SUM(PLANTID)/PLANTID AS LINECOUNT
FROM DWQ$LIBRARIAN.INV_SAP_PP_PO_HISTORY@ROCKWELL_DBLINK WHERE PLANTID  IN ('5050','5040', '5070', '5100', '5110', '5140', '5200')
GROUP BY EBELNPURCHDOCNO||'_'||MATERIALID,MATERIALID,PLANTID
)SPLIT_LINE_DELIVERY
...
SELECT *
FROM DWQ$LIBRARIAN.INV_SAP_SALES_STO_DTL
WHERE DEM_SOURCE     = 'SALES'
AND SHIPPING_PLANT   = '5050'
AND LINECREATIONDATE = '3-Jun-14';
...
WHERE PLANTID IN ('5050','5040', '5070', '5100', '5110', '5140', '5200') ---ALL PLANTS HERE!!
  ;
...
(SELECT MATERIALID
  ...
  ASOFDATE   AS LAST_REVIEW_DATE
FROM DWQ$LIBRARIAN.INV_SAP_INVENTORY_BY_PLANT@ROCKWELL_DBLINK
WHERE PLANTID IN ('5050') --- ALL PLANTS HERE!!
)INV_BASIC
LEFT JOIN
(
SELECT MATERIALID
...
FROM
  ...
  WHERE PLANT IN ('5050') ---All Plants
  GROUP BY MATERIAL
    ||'_'
    ||PLANT,
    MATERIAL,
...
FROM DWQ$LIBRARIAN.INV_SAP_PP_PO_HISTORY@ROCKWELL_DBLINK
WHERE PLANTID         IN ('5050')  ---All Plants
AND PO_SO_SA_FLAG     = 'STO'
AND DELIVERYCOMPLETE IS NULL
GROUP BY MATERIALID,
  PLANTID,
...
ON STO_SALES_OPEN_QTY.ID = INV_ITEM_BASIC.ID;
